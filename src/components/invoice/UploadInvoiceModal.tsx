...
const UploadInvoiceModal: React.FC<UploadInvoiceModalProps> = ({ 
  ...
  const simulateProcessing = () => {
    ...
    setTimeout(() => {
      // In a real app, we would send the file to a backend for processing
      // and receive a response with the extracted data
      const mockInvoiceData = {
        id: `INV-${Math.floor(1000 + Math.random() * 9000)}`,
        vendor: file?.name.split('.')[0] || 'Uploaded Vendor',
        amount: Math.floor(100 + Math.random() * 5000) + Math.random().toFixed(2),
        date: new Date().toISOString().split('T')[0],
        status: 'Pending',
        gstId: `GST${Math.floor(10000000000 + Math.random() * 90000000000)}`,
        category: Math.random() > 0.5 ? 'sales' : 'expense',
        tags: ['uploaded', 'new'],
        language: 'en',
        currency: 'USD',
        items: [
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a PDF or image file of your invoice to automatically extract the information.
          </DialogDescription>
        </DialogHeader>
          ...
          {processingStatus === 'error' && (
            <div className="flex flex-col items-center justify-center gap-4 py-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  We couldn't process your invoice. Please try again or upload a different file.
                </p>
              </div>
            </div>
          )}
